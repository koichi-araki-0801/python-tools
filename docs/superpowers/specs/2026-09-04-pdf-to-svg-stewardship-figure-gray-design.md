...
3. 手順 4 は 3 ペイン構成になる。
   ...
   採用一覧（`#fig-selist-box` / `#fig-selist`）は実装済み: 採用済みの図を
   書き出しファイル名（`<stem>_p<N>_fig<k>_gray.svg`）と pt 寸法（`W×H`）で
   全ページ分列挙する（書き出し範囲の設定に関係なく常に全件。範囲の設定は
   書き出し対象を絞るだけで一覧の見た目には影響しない）。行をクリックすると
   そのページへ移動する。
...
- 候補のクリックで採用/解除。採用済みの角ハンドルをドラッグで伸縮。空白ドラッグは
  `figure.js` の `installFigDrag` が担う（手順 3 の `installCropDrag` とは独立。矩形は
  ページ内へクランプする）。
- 手順 4 に入った時点で全ページの `figureCandidates` を順に取得し（フッターに
  「図を探しています i/N」）、検出できたページの矩形を採用済みとして `S.figSel`
  に入れ、最初に検出できたページへ自動で移動する。取得済みのページは再取得
  しない。利用者が既に触ったページの採用は上書きしない。全ページで検出ゼロ
  なら手動指定へ誘導する。
...
- `clip` 不正は `ValueError` にし、ディスパッチャが `{ok:false, error}` で返す（既存規約）。クライアントはトーストで通知する。
