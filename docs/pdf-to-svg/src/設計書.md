...
- 手順 1: ドロップゾーン下のチェックボックス（`#gray-mode-box`）。ON にするとステップバーの手順 2・3 に取消線が付き「手順 2・3 は省略されます」（`#gray-skipnote`）が表示される。手順 4 のラベルも「図をグレーで書き出す」（`#step4-label`）に変わる。
...
- ページ初表示時に `figureCandidates` RPC を取得し、1 件あれば採用済みとして `S.figSel` に入れる（既に利用者が触ったページは上書きしない）。検出ゼロのページは候補なしで始まり、手動ドラッグで追加できる。
