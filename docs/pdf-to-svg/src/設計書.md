...
| `cover.js` | 手順 3「上書き」ツールのオーバーレイ（移動・角ハンドル伸縮・置換語の変更）。矩形操作のヘルパ（角ハンドルの伸縮計算 `resizeByCorner` とハンドルのマークアップ `CORNER_HANDLES_HTML` を含む）は `geometry.js` から読み、複製しない（手順 4 の採用矩形と同じ実装） |
| `rpc.js` | `window.rpc(method, args)` = `/rpc` への fetch ラッパ |
| `dom.js` | 純粋ヘルパ（アイコン生成・エスケープ） |
| `geometry.js` | ページ座標・矩形操作・ページ指定パースのヘルパ。`clientToPage` / `pageSizeOf` は `viewBox` と要素の実寸を読み、`placeRect` は箱の `style` を書くため、`copyRect` / `clampToPage` / `rectIoU` / `parseSpec` / `MIN_SIZE_PT` / `resizeByCorner` / `CORNER_HANDLES_HTML` を含めても「純粋」とは呼ばない。矩形操作のヘルパは、角ハンドルの伸縮計算（`resizeByCorner`）とハンドルのマークアップ（`CORNER_HANDLES_HTML`）を含めて、手順 3 の上書き（`cover.js`）と手順 4 の採用矩形（`figure.js`）が同じ実装として共有する。`resizeByCorner` は掴んだ角を新しい点へ動かし反対の角を固定した新しい矩形を返す純粋関数で、渡された矩形そのものは書き換えない。手順 4（`figure.js`）は採用矩形の箱を配列の同一性で引くため矩形オブジェクトを置き換えられず、返り値を `Object.assign` で中身だけ書き込んで受ける。手順 3（`cover.js`）にはその制約が無く、返り値をそのまま代入する |
| `styles.css` | スタイル |
...
- ステップ 3（削除・枠線の編集）: ツールは「選択 / 範囲削除 / 枠線 / 上書き」。範囲削除・枠線はドラッグ矩形 → `deleteRegion` / `addBorder` RPC。「上書き」（`data-tool="cover"`）は元の文字が読み取れないページ向けのフォールバックで、`border-opts` と同じ位置に出る `cover-opts` の入力欄（`#cover-text`。プレースホルダ「置換語（空なら矩形だけ）」）に置換語を入れてから範囲をドラッグすると、mouseup で `addCover` RPC を呼ぶ（ドラッグの仕組みは `border` と共用し、`mode === "cover"` の分岐を足すだけ）。置いた上書きは `cover.js` の `drawCoverOverlay` が `coverList` の返り値をもとに角ハンドル 4 つ + 本体の HTML オーバーレイ（`.cover-box`）で重ね、角ハンドルのドラッグで伸縮・本体のドラッグで移動（いずれもページ内へクランプし、`MIN_SIZE_PT` 未満にはしない）、mouseup の 1 回だけ `updateCover {rect}` を送る。オーバーレイをクリックして選ぶと `#cover-text` にその上書きの語が入り、Enter または `change`（フォーカスが外れたとき）で `updateCover {text}` を送る（選んでいる間は入力欄への打鍵が「次に置く語」`S.coverText` を汚さない）。削除は「選択」ツールで選んで通常の削除ボタンを使う（削除一覧には「上書き「語」」と表示される）。
