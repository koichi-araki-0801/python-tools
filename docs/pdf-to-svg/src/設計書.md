...
rev:
  ...
  - 2.23 | 2026-09-18 | 手順 2・3 のページごとの確認（要確認 / 確認済み / スキップ）と確認バー・「スキップを除く」を撤去し、state の matches2 / edits3 でレールと右パネルを組み直した。ページへ移動する部品（page-jump.js）、手順 1 のバナー・バッジ・書き出し方の 2 択、手順 4 のまとめの表、Undo / Redo の文字ラベルを追記
  - 2.24 | 2026-09-18 | 残作業の整理（ページ送りの 1 本化、右パネルの行から削除するときの選択解除、レールの空表示の文言の 1 本化、グレーカードの説明文）を反映し、13 章に `state` / `pageSvg` / `planPage` の所要の実測値と、最適化を見送った判断を追記
audience: spec
---
...
flowchart TD
  ...
  UI --> EXP["SVG 直列化<br/>export/svg_exporter"]
...
確認一覧（`planPage`）は置換済み（`applied`）と未適用の候補（`pending`）を要素の出現順に混ぜて返し、`state` の `matches2` も同じ述語（`_page_match_counts`）で `[置換済み, 未置換]` を数える。数え方をそろえるのは、一覧の行数とレールのタグ・件数カード・手順 4 のまとめが食い違わないようにするためで、戻した箇所も未置換として残るのでページがレールの「辞書に一致したページだけ」から消えることもない。UI はこの順で通し番号を振り、表示中の SVG に `g[data-editor-marks]` として番号マーカーを描く（SVG 座標系なのでズームに追随。表示用 DOM のみで、書き出し SVG はサーバの `exportSvg` が別に生成するため混入しない）。
...
- `state` が返すのは、ファイル一覧 `files` / ページ一覧 `pages` / 総ページ数 `total` に加えて、ページごとの件数と属性の列である: `matches2`（ページごとの `[置換済み, 未置換]`。辞書に一致した箇所の数）、`edits3`（ページごとの `[削除, 枠線, 上書き]`）、`scanned`（純スキャンページかどうか）、`scannedPages`（その件数）、`truncated` / `noBackground` / `ocrPages`。ページごとの確認状態は返さない（画面もそれを持たない。8 章）。UI はこの件数からレールのタグ・右パネル・手順 4 のまとめを毎回描く。`ocrPages`（不可視の OCR 文字層を持つページ数。手動の上書きだけのページは数えない）は `truncated` / `noBackground` と同じ通知経路（`app.js` のトースト）に乗せる。
...
| ファイル | 役割 |
|---|---|
| `index.html` | 4 ステップ画面のマークアップ・ステップバー・フッター |
| `app.js` | 4 ステップ状態機械の本体。全状態・描画・編集・書き出し |
| `state.js` | 状態オブジェクト `S` と純粋な状態操作関数（グレーモードの遷移・書き出し対象列挙を含む。8.1 節） |
| `rail.js` | 左ページレール（一覧・絞り込み・件数タグ）の描画と配線。手順 2（`#pagenav`）と手順 3（`#pagenav-3`）が同一実装を共有する。行のタグは `S.matches2` / `S.edits3` から毎回描き、ページの確認状態は持たない。行が 1 つも出ないときの文言は手順で分けず、定数 `EMPTY_NOTE`（「この絞り込みに該当するページはありません」）1 本で出す |
| `page-jump.js` | 手順 2・3 のキャンバス下の「ページへ移動」部品（`#pgnav-2` / `#pgnav-3`）。ファイル選択 + ページ番号 + 「移動」+ 前後ボタンで、レールの絞り込みに出ていないページへも移れる。番号をファイル内番号から通し index へ丸める `resolveJump` は `S` を読まない純粋関数 |
...
- 手順を移るとき（`tryNext`・`back`・ステップバー・`advancePhase`）は `state.js` の `resetPhaseUi` を通し、要素の選択（`S.elSel`）・上書きの選択（`S.coverSel`）・枠線の選択（`S.borderSel`）・ツール（`S.tool`）を既定へ戻す。ツールの既定は無選択（`null`）である。手順をまたいで残すと、戻った直後のクリックが残っていた選択を外して削除ボタンが効かなくなり、ツールも「上書き」や「範囲削除」のまま始まるためである。ステップバーで今いる手順を押しただけのときは初期化しない。戻る方向（`back` とステップバー）では表示中のページ `S.page` を保ち、手順 4 で気付いた直しへそのページのまま戻れるようにする（ページ番号は全手順で共通の通し index なので、そのまま引き継げる）。進む方向の手順 2→3 と、手順 1 からの読み込み直後は従来どおり先頭ページから始める。ページ一覧の絞り込み（`S.filterFor`）は戻っても変えない。既定は手順 2 が「辞書に一致したページだけ」、手順 3 が「すべてのページ」で、ページの確認状態は無い。
- ステップ 2（用語置換）: 置換一覧の行クリックで該当箇所をフラッシュ、SVG の文字クリックで `dictSuggest` → 辞書「元の語」へ自動取り込み。「クリック取り込みで折返しを連結」チェック ON なら折返し 2 行を連結して取り込み、そのエントリに連結由来（`joined`）を記録する（一覧に「連結」バッジ表示）。再適用は「このページのみ」が主ボタン、「全ファイル」は明示選択。右パネル「確認」タブは、そのページで置き換えた語の数を出す件数カード（`.count-card`。中立の背景で、0 件でも緑にしない）と、一致箇所の一覧（`.change-row`）の 2 段構成で、下部に「前のページ」「次の一致ページ」（`nextMatched`。表示中より後ろで一致のあるページへ。無ければ先頭から探す）を置く。
- ステップ 3（削除・枠線の編集）: ツールは「範囲削除 / 枠線 / 上書き」の 3 つで、初期状態は無選択。押下中のタブをもう一度押すと無選択へ戻る。**要素のクリック選択はツールに関係なく常に効き**（ドラッグだけがツール固有の操作）、無選択のときはドラッグが何も起こさない。ドラッグで矩形を引いた直後にも `click` は飛ぶため、`S.dragMoved` を立ててその 1 回を握り潰す（握り潰さないと、引き終わった位置の要素が意図せず青枠になる）。このフラグのリセットは `wireStatic` が張る **window の capture フェーズの `mousedown` 1 箇所**に一元化する（次の `mousedown` が来た時点で必ず用済みになるため）。オーバーレイの箱・ハンドルの `mousedown` は自身の `stopPropagation()` でキャンバス側の `mousedown` ハンドラへ届かないため、個別の経路にリセットを足す形だと経路を 1 つ見落とすたびにクリック 1 回が空振りする不具合を繰り返す。3 ツールはいずれも、ドラッグの始点・終点を `geometry.js` の `rectFromDrag`（引いた向きを正規化しページ内へ収め、`MIN_SIZE_PT` 未満なら誤クリックとして捨てる）へ通してから矩形を確定し、それぞれ `deleteRegion` / `addBorder` / `addCover` RPC を呼ぶ（RPC が失敗したら `toast` で通知する）。範囲削除はページ外の矩形でも要素との交差判定にしか使わないため無害、枠線は矩形自体が SVG に残るのでページ内へ収まっている方が正しい（上書きは元々ページ内がサーバ検証の前提）。右パネルは `removedList` / `borderList` / `coverList` の 3 本を引いて「このページの編集」の一覧（`.edit-row[data-kind]`。削除は「戻す」、枠線と上書きは「削除」）を描き、その下に「使い方」（`HOWTO`。表示中のページがスキャン画像かどうかで文言を切り替える）を置く。パネル下部の「前のページ」「次のページ」（`#prev-page-3` / `#next-page-3`）は通しページで ±1 し、端では無効にする（手順 2 の「次の一致ページ」とは違い、一致の有無を見ない。手順 3 はどのページも編集の対象だからである）。
...
- ステップ 4（書き出し）: 1 枚なら SVG を直接ダウンロード、複数枚なら `zipEntries` で **ZIP 1 ファイル**にまとめてダウンロード。まとめ（`#export-summary`）は `matchTotals` / `editTotals` から「PDF」「用語の置換」「削除・枠線」の 3 行を組む表（`.sum-table`。件数の項目は `white-space: nowrap` で、折り返しは項目の区切りでだけ起こす）。書き出す範囲は「表示中のページのみ / 全ページ / ページを指定」の 3 択である。

**ページごとの確認を持たない理由**: 手順 3 は編集する操作であって確認する操作ではなく、編集していないページはそのまま書き出される。手順 2 も辞書に一致したページを見て回るだけで足りる。確認状態を持つと、全ページに「確認しました」を求める手間と、絞り込みに出ないページが生まれる。
...
手順 1「PDF を選ぶ」の「書き出し方」で「図だけをグレースケールで書き出す」（`#mode-gray`）を選ぶと入る、通常モードとは別動線。用語置換・削除／枠線の編集を省き、検出した図の採用・伸縮だけを行って書き出す。
...
**遷移**: `tryNext` は phase 1 かつ `S.gray` のとき phase 4 へ直行する（`S.page = 0`）。`back` は phase 4 かつ `S.gray` のとき phase 1 へ戻る。ステップクリックは gray のとき 1 と 4 のみ許す。`advancePhase` 自体は変えない。書き出し範囲は gray のとき「表示中のページの図」（`page`）と「全ページの採用した図」（`all`）の 2 択（`#exp-modes-gray`）へ差し替え、通常モードの 3 択（`#exp-modes`）は隠す。書き出し対象の列挙 `exportFigureList()` は採用矩形を `{fileIndex, pageInFile, clip, figIndex}` に展開する（DOM 非依存。vitest で検証）。

**画面（3 ペイン。`index.html` / `figure.js`）**:

- 手順 1: ドロップゾーン下の「書き出し方」の 2 択カード（`input[type=radio][name=mode]` の `#mode-normal`（既定）と `#mode-gray`。囲みの `<label>` は `#mode-normal-box` / `#mode-gray-box` で、選択中のカードに `.mode-card.on` が付く）。カードにはモードの説明と流れ（通常は「1 PDF › 2 用語 › 3 削除・枠線 › 4 SVG」、グレーは「1 PDF › 4 図を選ぶ・SVG」）を出し、手順 2 を省略するときは通常側の「2 用語」（`#flow-step2`）に打ち消しを付ける。`#mode-gray` を選ぶとステップバーの手順 2・3 を非表示にし、「手順 2・3 は省略されます」（`#gray-skipnote`）の注記を出す。手順 4 のラベルも「図をグレーで書き出す」（`#step4-label`）に変わる。
...
**判定源**: サーバの `state` RPC が、ページ列と同じ長さの `scanned: bool[]`（`Page.is_scanned`）と件数 `scannedPages` を返す。クライアントは `applyState` でそれを `S.scanned` へそのまま写し、判定源をこの 1 本に閉じる（判定源を 2 本持たない。`scanned` を別の状態へ写さない——写した瞬間に食い違う余地ができる）。`scanned` はページの属性で変わらないため、編集のたびの再取得でも同じ値が返る。不可視 OCR 文字を持つページ（5.1 節の不可視 OCR 文字の 3 分岐）は `is_scanned` ではないので対象外にならない（OCR が正しければ辞書が当たる。外れたときの逃げ道が上書きツール）。

**レール・ページ送り**: レールに出すページを決める `railPages(phase)` が、手順 2 では絞り込み（`S.filterFor[2]`）に関わらずスキャンページを落とすため、行にもファイル行の件数にも出ない。ファイル内が全部スキャンならファイル行ごと出ない。手順 3 のレールは出す（上書きの対象になるため）。「次の一致ページ」（`nextMatched`）もスキャンページには着地しない。手順 2 に入る 3 経路（手順 1 の「次へ」・手順 3 の「戻る」・ステップバー）は `landOnPhase2()` を通し、表示中のページがスキャンなら最初の対象ページ（`firstEditablePage2()`）へ差し替える（対象ページに居るときは動かさず、戻ったときに見ていたページを保つ）。キャンバス下の「ページへ移動」（`page-jump.js`）はレールを経由しないので、手順 2 でもスキャンページへ番号で移れる。その場合は右パネルに「このページはスキャン画像のため、用語の置換の対象外です」を出す。

右パネル下部のページ送り（手順 2 の `#prev-page-2` / `#next-match-2` と手順 3 の `#prev-page-3` / `#next-page-3`）は、手順ごとに配線を写し取らず、次の行き先を返す関数だけを差し替えて `wirePageFoot(prevId, nextId, nextIndexFn)` の 1 本から配線する。手順 2 は `nextMatched(S.page)`、手順 3 は通し +1（末尾なら -1）を渡す。「前のページ」は両方とも通し −1 で、押下可否は `nextIndexFn()` が負を返すかどうかで決まるため、押せるかどうかの判定と移動が 1 箇所に閉じる。静的なボタンなので `onclick` 代入で配線し、再描画のたびにハンドラが積み上がらないようにする。ページごとの件数は `matchCount(g)`（`S.matches2` の適用済み + 未適用）と `editCount(g)`（`S.edits3` の削除 + 枠線 + 上書き）が数え、レールの絞り込み・「次の一致ページ」の巡回・手順 4 のまとめがこの 2 つを共有する（同じ足し算を呼び出し側ごとに書かない）。

**遷移**: `skipsPhase2()` = ページが 1 つ以上あり `S.scanned` がすべて true。`phaseAfterLoad()` は `gray ? 4 : (skipsPhase2() ? 3 : 2)`、手順 3 の「戻る」の行き先 `phaseBeforeTrim()` は `skipsPhase2() ? 1 : 2`、`stepAllowed(2)` は `skipsPhase2()` のとき false。グレーモードが常に優先する。手順 4 の「戻る」（`phaseBeforeExport`）は変えない。

**画面**: 案内は手順 1 に**常設**する。スキャンページが 1 ページ以上あれば、ファイル一覧の見出しとカードの間にバナー（`#scan-banner` / `#scan-banner-text`）を出す。文言は 2 系統で、全ページがスキャンなら「選んだ PDF はすべてスキャン画像です（N ページ）。」に続けて置換ができないこと・手順 2 を省略して「次へ」で手順 3 へ進むこと・手順 3 の「上書き」で置き換えられることを、混在なら「スキャン画像のページが N ページあります。」に続けて置換ができないこと（手順 2 の一覧には出ないこと）と上書きの逃げ道を、それぞれ 1 文 1 行で並べる。グレーモードのときは出さない（手順 2 を通らないため）。ファイルカードには、そのファイルのスキャンページ数のバッジ「スキャン画像 n / m ページ」（`.chip`。`scannedCountOf(fileIndex)`）を出す。`skipsPhase2()` のときはステップバーの 2 を隠し（`.stepbar.skip2`）「手順 2 は省略されます（スキャン画像のみのため）」（`#scan-skipnote`）を出す。手順 1 の「次へ」は**何も出さずに** `phaseAfterLoad()` の行き先（手順 3）へ進む。案内モーダルもトーストも持たない——読み込みの途中でファイルが増えれば案内の内容もページ数も変わるため、一度だけ見せる形だと「見た内容」と「進む先」がずれる。常設のバナーとバッジなら、その時点の状態が画面にそのまま出ている。手順 4 のまとめ（`#export-summary`）は、スキャンページが 1 ページ以上あるときだけ「対象外（スキャン画像） N ページ」の項目を足す。

**書き出し**: スキャンページは背景画像として従来どおり書き出され、手順 3 で置いた上書き・枠線も残る。ページを除外する手段は「ページを指定」だけで、「スキップを除く」（`noskip`）は持たない（スキャンページをまとめて落とす案は、上書きを置いたのに書き出されない退行になるため却下）。
...
ページごとの確認を持たない画面（8 章）と、純スキャンページを手順 2 の対象外にする仕組み（8.2 節）は、サーバ側を `test_web_rpc.py::test_state_counts_pending_candidates_and_reverted_matches` / `::test_state_counts_removed_borders_and_covers_per_page` が固定する（`state` の `matches2` が未適用の候補と戻した箇所を未置換として数えること、`edits3` が削除・枠線・上書きをページごとに数えること）。`::test_state_marks_scanned_pages_as_not_replaceable` が `scanned` 列と `scannedPages` を、`test_pdftosvg_state_js.py` の `test_railpages_phase2_matched_default_and_all_never_include_scanned` / `::test_railpages_phase3_all_default_includes_scanned_and_edited_filters`・`::test_nextmatched_searches_ahead_then_wraps_and_skips_scanned`・`::test_skipsphase2_only_when_every_page_is_scanned` ほかが、レールの絞り込みの既定とスキャンページの除外・「次の一致ページ」の巡回・遷移（`skipsPhase2` / `phaseAfterLoad` / `phaseBeforeTrim` / `stepAllowed` / `landOnPhase2`）を単体で固定する。書き出しモードが `page` / `all` / `spec` の 3 つだけであること（「スキップを除く」が無いこと）は `::test_export_modes_are_page_all_spec_only` が見る。「ページへ移動」は `test_pdftosvg_page_jump_js.py` が `resolveJump` の丸め（範囲外の番号・数字でない値・未知のファイル）を実ブラウザ単体で確かめ、実画面は `test_pdftosvg_app_flow_e2e.py::test_page_jump_moves_by_number_and_arrows_including_pages_hidden_from_the_rail` が番号入力・前後ボタン・レールに出ていないページへの移動まで確かめる。複数の PDF を読ませたときの振る舞い（ファイル選択を変えるとページ番号と上限がそのファイルの先頭・ページ数へ戻るだけで移動はせず「移動」で確定すること、前後ボタンは通しページなのでファイルの境をまたぎ末尾で無効になること）は、同じファイルの `::test_page_jump_file_select_resets_number_and_arrows_cross_files` が実 Edge で確かめる。手順 1 の案内は `::test_all_scanned_pdf_shows_banner_and_skips_step2_without_a_dialog` / `::test_mixed_scanned_and_vector_pdfs_hide_scanned_rows_without_a_dialog` が、常設バナーとファイルカードのバッジが出ること・モーダルが出ないこと・「次へ」が止まらずに手順 3 へ進むこと・混在ならレールにスキャン行が無いことを実 Edge で確かめる。
...
**state RPC の所要（2026-09-18 実測）**: 編集のたびに `state` を取り直し、ページ全体の件数を数え直す設計（8 章）が規模に耐えるかを、合成 PDF で測った。条件は 100 ページ（抽出後の要素は 1 ページ 27 個・文書合計 2,700 個）・辞書 55 語で、Python 3.13.15 + PyMuPDF 1.28.2、`time.perf_counter` の 5 回中央値である。結果は `state` が 10.0 ms（全ページ未置換）/ 8.8 ms（全 500 箇所を置換した後）、`planPage` が 0.1 ms、`pageSvg` が 0.5 ms（未置換）/ 120.9 ms（置換後）であった。`state` はページ内の要素を線形に舐めるためページ数・要素数に比例して伸びるが、1 ページ 270 要素（10 倍）を見込んでも 100 ms 前後であり、編集のたびに取り直しても体感に出る量ではない。したがって表示中のページだけを作り直す形への最適化（`S.svgCache` の作り替え）は行わず、毎回取り直す形をそのまま残している。置換の前後で `pageSvg` が 240 倍に伸びるのは、置換語に和文が入って同梱フォントの `@font-face` 埋込が要るようになるためで、cProfile ではこの 120.9 ms のほぼ全部（99%）が `font_embed._subset_woff2` の WOFF2 サブセット化であった。将来ここが体感の問題になったとき効くのは `state` の呼び出し回数ではなく、使用グリフ集合をキーにしたサブセット結果の再利用である。
