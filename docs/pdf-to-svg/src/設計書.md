...
- `clip` あり: `viewBox` と `width` / `height` を clip 矩形にし、clip に交差しない要素は出力しない。交差する要素のはみ出しは根直下の `<clipPath>`（標準要素なので Office でも効く）で切る。`clipPath` の属性も `_attr` 経由で組み立てる。
...
- 手順 1: ドロップゾーン下のチェックボックス（`#chk-gray`。囲みの `<label>` は `#gray-mode-box`）。ON にするとステップバーの手順 2・3 に取消線が付き「手順 2・3 は省略されます」（`#gray-skipnote`）が表示される。手順 4 のラベルも「図をグレーで書き出す」（`#step4-label`）に変わる。
...
- 手順 4 右（`#export-center`）: 件数、書き出し範囲（`#exp-modes-gray`）、書き出しボタン。
