...
| `rail.js` | 左ページレール（一覧・絞り込み・件数タグ）の描画と配線。手順 2（`#pagenav`）と手順 3（`#pagenav-3`）が同一実装を共有する。行のタグは `S.matches2` / `S.edits3` から毎回描き、ページの確認状態は持たない |
