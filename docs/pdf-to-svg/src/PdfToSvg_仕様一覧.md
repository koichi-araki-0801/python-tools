...
| 39 | `test_pdftosvg_app_flow_e2e.py::test_click_after_offpage_drag_and_overlay_select_selects_in_one_click`、`test_pdftosvg_rect_overlay_js.py` の `ro` fixture の teardown | ページ外へはみ出すドラッグで枠線を置いた直後にその箱をクリックして選んでも `S.dragMoved` が消費されずに残らないこと（`window.__state` で直接確認）、続けて要素をクリックすれば1回で選べること（E2E）。`rect-overlay.js` 単体テストの `S`（モジュールシングルトン）書き換えを、共有の `edge_page` を汚さないよう teardown で元へ戻す | オーバーレイの mousedown 経由でもクリック握り潰しが再発しない、テストの実行順に依存しない | 未 |
