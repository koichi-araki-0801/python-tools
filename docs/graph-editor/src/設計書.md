...
| 色 | 名称 | 位置 | 可動性 |
|---|---|---|---|
| 緑 `#57c878` | 端点 | 文字側（点列の末尾） | ドラッグ可（**ドラッグ中はクランプしない** — ラベル外枠の外へも置ける。次にラベルを動かすか行数・長体を変えた時点で `snapEndpointToFrame` が外枠上へ引き戻す） |
...
| 関数 | 役割 |
|---|---|
| `clampPointToBox(p, box)` | 点 p を矩形へクランプ（矩形外なら外枠上の最近点）。アプリが引出線端点をラベル外枠上へ載せ直すときの核（手動ハンドルのドラッグ中は呼ばない。設計正典「手動ハンドルのドラッグはクランプしない」を参照） |
...
- `scripts/build.bat` → `scripts/build.ps1`: 共有ライブラリ `scripts/lib/build-python-venv.ps1` で隔離 venv（`.venv-build`）を用意し、**PyInstaller `--onefile`** でビルドする:
